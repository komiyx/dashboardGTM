...
function saveDataToDatabase($username, $url) {
    ...
    $query = "SELECT id FROM th_user_dep_records WHERE username = ?";
    ...
    if ($stmt->num_rows > 0) {
        ...
        $updateQuery = "UPDATE th_user_dep_records SET url = ?, last_created_time = ? WHERE username = ?";
        ...
        $insertQuery = "INSERT INTO th_user_dep_records (username, url, last_created_time) VALUES (?, ?, ?)";
