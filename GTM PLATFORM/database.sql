...
CREATE TRIGGER after_aud_user_dep_records_insert
AFTER INSERT ON aud_user_dep_records
FOR EACH ROW
BEGIN
    -- Check if the username in aud_user_records matches the newly inserted username
    UPDATE aud_user_records
    SET deposit_status = 'YES'
    WHERE username = NEW.username;
END$$
...
CREATE TABLE aud_user_dep_records (
    id INT AUTO_INCREMENT PRIMARY KEY,
    username VARCHAR(255) NOT NULL UNIQUE,
    url VARCHAR(255) NOT NULL,
    last_created_time DATETIME NOT NULL
...
CREATE TABLE hk_user_dep_records (
    id INT AUTO_INCREMENT PRIMARY KEY,
    username VARCHAR(255) NOT NULL UNIQUE,
    url VARCHAR(255) NOT NULL,
    last_created_time DATETIME NOT NULL
